...
namespace Opcua.Utils
{
    public partial class ClientUtils
    {
        ...
        public static string GetAttributeDisplayText(Session session, uint attributeId, Variant value)
        {
            ...
            {
                return Opc.Ua.Utils.ToHexString(value.Value as byte[]);
            }
            ...
            IDictionary<string, InstanceDeclaration> map)
        {
            ...
            for (int ii = 0; ii < references.Count; ii++)
            {
                ...
                if (parent != null)
                {
                    ...
                    child.BrowsePathDisplayText = Opc.Ua.Utils.Format("{0}/{1}", parent.BrowsePathDisplayText, reference.BrowseName);
                    child.DisplayPath = Opc.Ua.Utils.Format("{0}/{1}", parent.DisplayPath, reference.DisplayName);
                }
                else
                {
                    child.BrowsePath = new QualifiedNameCollection();
                    child.BrowsePathDisplayText = Opc.Ua.Utils.Format("{0}", reference.BrowseName);
                    child.DisplayPath = Opc.Ua.Utils.Format("{0}", reference.DisplayName);
                }
